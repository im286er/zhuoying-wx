...
class Model {
     ...
     */
     public function create($data='',$type='') {
        ...
        if(empty($data)) {
            $data   =   I('post.');
        }elseif($data == 'json') {//Add By NanQi 增加对于客户端请求json数据的自动创建数据处理
            $data   =   I('json.');
    ...
    public function regex($value,$rule) {
        $validate = array(
            ...
            'phonenumber'   =>  '/1[34578]{1}\\d{9}$/',
        );

        // 检查是否有内置的正则表达式
        if(isset($validate[strtolower($rule)])) {
            $errCode = C('ERROR_CODE');
            $this->errorCode = $errCode[strtolower($rule)];
     ...
     * @param null $data 需要验证的数据，不传直接从请求参数中获取
     * @return boolean 验证失败直接返回错误
     */
    public function validation($validate, $data = null) {
        ...
        // 验证数据
        if(empty($data) || !is_array($data)) {
            $data['MYLINE_PLACEHOLDER'] = null;
        }

        if (!$this->validate($validate)->autoValidation($data, null)) {
    ...
    protected function _validationFieldItem($data,$val) {
        ...
        switch(strtolower(trim($val[4]))) {
            ...
            case 'callback':// 调用方法进行验证
                $this->errorCode = $errCode['function'];
            ...
            case 'confirm': // 验证两个字段是否相同
                $this->errorCode = $errCode['confirm'];
                return $data[$val[0]] == $data[$val[1]];
            case 'unique': // 验证某个值是否唯一
                $this->errorCode = $errCode['unique'];
                if(is_string($val[0]) && strpos($val[0],','))
                    $val[0]  =  explode(',',$val[0]);
                $map = array();
    ...
    public function check($value,$rule,$type='regex'){
        ...
        switch($type) {
            ...
            case 'notin':
                $this->errorCode = $errCode['in'];
                $range   = is_array($rule)? $rule : explode(',',$rule);
                return $type == 'in' ? in_array($value ,$range) : !in_array($value ,$range);
            case 'between': // 验证是否在某个范围
            case 'notbetween': // 验证是否不在某个范围            
                $this->errorCode = $errCode['between'];
                if (is_array($rule)){
                    $min    =    $rule[0];
                    $max    =    $rule[1];
                }else{
            ...
            case 'notequal': // 验证是否等于某个值            
                $this->errorCode = $errCode['equal'];
                return $type == 'equal' ? $value == $rule : $value != $rule;
            case 'length': // 验证长度
                $this->errorCode = $errCode['length'];
