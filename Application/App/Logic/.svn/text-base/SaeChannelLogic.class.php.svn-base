...
class SaeChannelLogic extends BaseLogic {
     ...
     */
     public function createChannel( $name, $duration = 3600)
     {
         ...
         $request_url = SaeChannelModel::end_.'create_channel';
     ...
     */
     public function sendMessage( $name, $message)
     {
         ...
         $request_url = SaeChannelModel::end_.'send_message';
    ...
    */
     ...
     */
     ...
     {
         if ( strlen( $client_id ) > SaeChannelModel::MAXIMUM_CLIENT_ID_LENGTH_ ) {
             $errmsg = sprintf('Client id length %d is greater than max length %d',strlen($client_id), SaeChannelModel::MAXIMUM_CLIENT_ID_LENGTH_ );
    ...
    */
     private function check_duration( $duration )
     {
         if (!$duration) {
             ...
             } elseif (strlen($duration) > SaeChannelModel::MAXIMUM_TOKEN_DURATION_SECONDS_) {
                 $errmsg = sprintf('Argument duration must be less than %d',(SaeChannelModel::MAXIMUM_TOKEN_DURATION_SECONDS_ + 1));
             }
             if ( !is_null($errmsg)) {
                 $this->set_error(101,$errmsg);
                 return false;
     ...
     */
     private function check_message( $message )
     {
         ...
         } elseif ( strlen($message) > SaeChannelModel::MAXIMUM_MESSAGE_LENGTH_) {
             $errmsg = sprintf('Message must be no longer than %d chars' ,SaeChannelModel::MAXIMUM_MESSAGE_LENGTH_);
         }
         if (!is_null($errmsg)) {
             $this->set_error(101,$errmsg);
             return false;
