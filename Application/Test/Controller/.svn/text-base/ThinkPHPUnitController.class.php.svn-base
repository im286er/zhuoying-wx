...
abstract class ThinkPHPUnitController extends \Think\Controller
{
    ...
    {
        $log = <<<DATA
<b>[Failure]</b>: {$trace['message']}<br/>
    <b>CLASS</b>: {$trace['class']}<br/>
    <b>METHOD</b>: {$trace['method']}<br/>
    <b>LINE</b>: {$trace['line']}<br/>
    <hr/>
\n
DATA;
        echo $log;
...
DATA;
        ...
        if (is_a($e, 'PHPUnit_Framework_ExpectationFailedException')) {
            ...
            foreach ($e->getTrace() as $trace) {
                if ($trace['class'] == $this_action_class_name) {
                    $action_trace = $trace;
                }elseif ('Test\\Controller\\ThinkPHPUnitController' == $trace['class'] && '__call' == $trace['function']) {
                    $assert_trace = $trace;
                }
            }
    ...
    public function index()
    {
        ...
        foreach($methods as $method) {
            if (preg_match('/^test_?/', $method) && is_callable(array($this, $method))) {
