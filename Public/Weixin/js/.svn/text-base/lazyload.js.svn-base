define(function(require, exports, module) {
	...
	mui.Lazyload = mui.Class.extend({
		...
		_initLoadFn: function() {
			...
			self._loadFn = this._buffer(function() { // 加载延迟项
				if (self.options.autoDestroy && self._counter == 0 && mui.isEmptyObject(self._callbacks)) {
					self.destroy();
				}
				self._loadItems();
		...
		_buffer: function(fn, ms, context) {
			...
			return mui.extend(function() {
				if (
					(!lastStart) || // 从未运行过
					(lastEnd >= lastStart && mui.now() - lastEnd > ms) || // 上次运行成功后已经超过ms毫秒
					(lastEnd < lastStart && mui.now() - lastStart > ms * 8) // 上次运行或未完成，后8*ms毫秒
				) {
					run();
				} else {
					if (timer) {
					...
					timer = mui.later(run, ms, null, arguments);
				}
			}, {
				stop: function() {
					if (timer) {
		...
		_elementInViewport: function(elem, windowRegion, containerRegion) {
			...
			var elemOffset = mui.offset(elem);
		...
		_loadItems: function() {
			...
			mui.each(self._callbacks, function(key, callback) {
		...
		addElements: function(elements) {
			...
			if (!elements && self.options.selector) {
				...
				mui.each(elements, function(index, el) {
					lazyloads = lazyloads.concat(mui.qsa(self.options.selector, el));
				});
			}
			mui.each(lazyloads, function(index, el) {
		...
		destroy: function() {
			...
			self.pause();
			self._callbacks = {};
			mui.trigger(this.container, 'destory', self);
			self._destroyed = 1;
		}
	});
	
	var ImageLazyload = mui.Lazyload.extend({
		init: function(element, options) {
	...
	mui.fn.imageLazyload = function(options) {
		var lazyloadApis = [];
		this.each(function() {
			var self = this;
			...
			if (!id) {
				id = ++mui.uuid;
				mui.data[id] = lazyloadApi = new ImageLazyload(self, options);
				self.setAttribute('data-imageLazyload', id);
			} else {
				lazyloadApi = mui.data[id];
			}
